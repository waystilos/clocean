...
export const DatabasesView: React.FC<Props> = ({ workspaceId, currentUser, getAuthHeaders }) => {
  ...
  const newDatabaseInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const loadDatabases = async () => {
    ...
    try {
      ...
      setSelected((current) => data.databases.find((database) => database.id === current?.id) ?? data.databases[0] ?? null);
    } finally {
  ...
  useEffect(() => {
    ...
    setRecords([]);
    setError("");
    loadDatabases().catch((e) => setError(e.message));
  }, [workspaceId]);
  ...
  const createDatabase = async () => {
    ...
    if (!name) {
      ...
      newDatabaseInputRef.current?.focus();
      return;
    }
  ...
  return <div className="animate-fade-in database-view" style={{ padding: "32px", maxWidth: "1200px", margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
    ...
    {error && <div role="alert" aria-live="polite" style={{ color: "var(--danger, #b42318)", marginBottom: 16, padding: "10px 12px", borderRadius: 8, background: "rgba(220, 38, 38, 0.08)", border: "1px solid rgba(220, 38, 38, 0.2)" }}>{error}</div>}
    <div className="database-layout" style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 24 }}>
      <aside style={{ background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 12, padding: 12 }}><strong style={{ padding: 8, display: "block" }}>Your databases</strong>{isLoadingDatabases ? <div style={{ padding: 8, color: "var(--text-secondary)", fontSize: 13 }}>Loading databases…</div> : databases.map((database) => <button key={database.id} onClick={() => setSelected(database)} style={{ display: "flex", gap: 8, width: "100%", textAlign: "left", padding: 10, border: 0, borderRadius: 8, color: "var(--text-primary)", background: selected?.id === database.id ? "var(--bg-nav-active)" : "transparent" }}><Table2 size={16} />{database.name}</button>)}{!isLoadingDatabases && databases.length === 0 && <span style={{ padding: 8, color: "var(--text-secondary)", fontSize: 13 }}>Your first database will appear here.</span>}</aside>
      <section className="database-panel" style={{ background: "var(--bg-surface)", border: "1px solid var(--border-subtle)", borderRadius: 12, overflow: "hidden" }}>{selected ? (<><div className="database-toolbar" style={{ padding: 16, display: "flex", justifyContent: "space-between", gap: 12, borderBottom: "1px solid var(--border-subtle)" }}><h2 style={{ margin: 0, fontSize: 20 }}>{selected.name}</h2><div className="database-actions" style={{ display: "flex", gap: 8 }}><label style={{ display: "flex", alignItems: "center", gap: 6 }}><Search size={15} /><input value={search} maxLength={100} onChange={(e) => setSearch(e.target.value)} placeholder="Filter rows" aria-label="Filter rows" /></label><input value={newRecordTitle} maxLength={200} onChange={(e) => setNewRecordTitle(e.target.value)} placeholder="New row" aria-label="New row title" /><button className="btn-primary" onClick={createRecord}><Plus size={16} /> Add</button></div></div><div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={{ textAlign: "left", padding: 12 }}>Name</th>{columns.map((column) => <th key={column.id} style={{ textAlign: "left", padding: 12 }}>{column.name}</th>)}<th /></tr></thead><tbody>{records.map((record) => <tr key={record.id} style={{ borderTop: "1px solid var(--border-subtle)" }}><td style={{ padding: 12 }}>{record.title}</td>{columns.map((column) => <td key={column.id} style={{ padding: 12, color: "var(--text-secondary)" }}>{Array.isArray(record.properties[column.id]) ? (record.properties[column.id] as string[]).join(", ") : String(record.properties[column.id] ?? "")}</td>)}<td style={{ padding: 12, textAlign: "right" }}><button aria-label={`Delete ${record.title}`} onClick={() => deleteRecord(record.id)} style={{ border: 0, background: "transparent", color: "var(--text-secondary)" }}><Trash2 size={16} /></button></td></tr>)}{records.length === 0 && <tr><td colSpan={columns.length + 2} style={{ padding: 32, textAlign: "center", color: "var(--text-secondary)" }}>No rows yet.</td></tr>}</tbody></table></div></>) : (<div className="database-empty-state" style={{ padding: 48, textAlign: "center", color: "var(--text-secondary)" }}><Table2 size={36} color="var(--accent)" /><h2 style={{ margin: "14px 0 8px", color: "var(--text-primary)", fontSize: 22 }}>Start with a database</h2><p style={{ maxWidth: 420, margin: "0 auto 18px", lineHeight: 1.6 }}>Create a structured table for projects, content, or anything your workspace needs. Your first database starts with Status and Due date fields.</p><button className="btn-primary" onClick={() => newDatabaseInputRef.current?.focus()}><Plus size={16} /> Create your first database</button></div>)}</section>
    </div>
